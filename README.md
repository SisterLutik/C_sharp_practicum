...
В приложении используются два механизма синхронизации для обеспечения потокобезопасности:

1. lock (в BookingService)
   - Защищает критическую секцию при создании брони
   - Обеспечивает атомарность операции "проверка мест + резервирование"
   - Предотвращает овербукинг (два потока не могут одновременно забронировать одно место)

2. SemaphoreSlim (в BookingBackgroundService)
   - Асинхронный аналог Mutex, используется с await
   - Защищает запись в хранилище при параллельной обработке броней
   - Позволяет выполнять длительные операции (Task.Delay) параллельно, а запись — последовательно
...
{
    "status": 409,
    "detail": "Нет свободных мест для события 3f2c1d8e-1234-5678-9abc-def012345678"
}
...
Сценарий с овербукингом (защита от конкурентного доступа)

При одновременном бронировании мест несколькими пользователями система гарантирует, что количество успешных броней не превышает количество доступных мест.

Пример:
- Событие: 3 места
- 5 параллельных запросов на бронирование
- Результат: ровно 3 успешные брони (статус Pending)
- Остальные 2 запроса получают ответ 409 Conflict
- availableSeats становится 0

Это достигается за счёт lock в BookingService, который делает атомарной операцию "проверить места + уменьшить количество".
